...
function isCreditCardNumber(ccn) {
  ...
  const cleanedCCN = ccnString.replace(/\D/g, '');
  const reversedCCN = cleanedCCN.split('').reverse().join('');
  let sum = 0;
...
function getDigitalRoot(num) {
  ...
  while (sum > 9) {
    const digitArray = sum.toString().split('');
    sum = digitArray.reduce((acc, digit) => acc + Number(digit), 0);
  }
...
function getCommonDirectoryPath(pathes) {
  const firstPathSegments = pathes[0].split('/');
  let commonPath = '';

  for (let i = 0; i < firstPathSegments.length; i += 1) {
    const segment = firstPathSegments[i];
    const isInAllPaths = pathes.every((path) => {
